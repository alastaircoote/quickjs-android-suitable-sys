...
fn generate_bindings() {
    ...
    let header_path = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap()).join("quickjs+extern.h");
    let out_path = PathBuf::from(env::var("OUT_DIR").unwrap());
    
    // Generate bindings.
    let bindings = bindgen::Builder::default()
        .header(header_path.to_str().expect("Could not create QuickJS path").to_string())
